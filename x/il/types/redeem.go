...
var (
	_ Payload = &RedeemLiquidityETH{}
	_ Payload = &RedeemLiquidity{}
	_ Payload = &SimpleLogicBatch{}
)
...
// NewRedeemLiquidityETHCall
func NewRedeemLiquidityETHCall(token string, liquidity, amountTokenMin, amountEthMin uint64, to string, deadline int64) *RedeemLiquidityETH {
	return &RedeemLiquidityETH{
		Token:          common.HexToAddress(token),
		Liquidity:      new(big.Int).SetUint64(liquidity),
...
func NewRedeemLiquidityCall(tokenA, tokenB string, liquidity, amountAMin, amountBMin uint64, to string, deadline int64) *RedeemLiquidity {
	return &RedeemLiquidity{
		TokenA:     common.HexToAddress(tokenA),
		TokenB:     common.HexToAddress(tokenB),
		Liquidity:  new(big.Int).SetUint64(liquidity),
		AmountAMin: new(big.Int).SetUint64(amountAMin),
		AmountBMin: new(big.Int).SetUint64(amountBMin),
		To:         common.HexToAddress(to),
		Deadline:   big.NewInt(deadline),
	}
}
...
// GetEncodedCall gets the checkpoint signature from the given outgoing tx batch
func (b RedeemLiquidity) GetEncodedCall() ([]byte, error) {
	abi, err := abi.JSON(strings.NewReader(testUniswapLiquidityABI))
	if err != nil {
		return nil, sdkerrors.Wrap(err, "bad ABI definition in code")
	}
	...
	return abi.Pack(
		"redeemLiquidity",
		b.TokenA,     // address tokenA
		b.TokenB,     // address tokenB
		b.Liquidity,  // uint256 liquidity
		b.AmountAMin, // uint256 amountAMin
		b.AmountBMin, // uint256 amountBMin,
		b.To,         // address to,
		b.Deadline,   // uint256 deadline
	)
}
...
func (b SimpleLogicBatch) GetEncodedCall() ([]byte, error) {
	...
	for i, p := range b.Payloads {
		...
		if err != nil {
			return nil, sdkerrors.Wrapf(err, "invalid payload at index %d", i)
		}
