...
func InitGenesis(ctx sdk.Context, keeper keeper.Keeper, data types.GenesisState) {
	...
	if len(data.TobinTaxes) > 0 {
		for _, tobinTax := range data.TobinTaxes {
			keeper.SetTobinTax(ctx, tobinTax.Denom, tobinTax.ExchangeRate)
		}
	} else {
...
func ExportGenesis(ctx sdk.Context, keeper keeper.Keeper) (data types.GenesisState) {
	...
	keeper.IterateLunaExchangeRates(ctx, func(denom string, rate sdk.Dec) (stop bool) {
		rates = append(rates, &types.ExchangeRateTuple{Denom: denom, ExchangeRate: rate})
		return false
	})
	...
	keeper.IterateTobinTaxes(ctx, func(denom string, tobinTax sdk.Dec) (stop bool) {
		tobinTaxes = append(tobinTaxes, &types.ExchangeRateTuple{Denom: denom, ExchangeRate: tobinTax})
		return false
	})
