...
func TestScheduledCorkProposalValidation(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expPass: false,
			err:     errorsmod.Wrapf(corktypes.ErrInvalidEthereumAddress, "0x01"),
		},
		{
			...
			expPass: false,
			err:     errorsmod.Wrap(corktypes.ErrInvalidJSON, "cannot have empty contract call"),
		},
		{
			name: "Invalid JSON",
			scheduledCorkProposal: ScheduledCorkProposal{
			...
			expPass: false,
			err:     errorsmod.Wrapf(corktypes.ErrInvalidJSON, "[}"),
		},
	}
