...
func GenTxCmd(mbm module.BasicManager, txEncCfg client.TxEncodingConfig, genBalIterator types.GenesisBalancesIterator, defaultNodeHome string) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "gentx [key_name] [amount] [eth-address] [orchestrator-address] [oracle-address]",
...
`, defaultsDesc, version.AppName,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			oracleAddress, err := sdk.AccAddressFromBech32(args[4])
			if err != nil {
				return errors.Wrapf(err, "failed to parse oracleAddress(%s)", args[4])
			}
			...
			// validate oracle account in genesis and warn if not found
			if err = genutil.ValidateAccountInGenesis(genesisState, genBalIterator, oracleAddress, coins, cdc); err != nil {
				cmd.PrintErrf("oracle address not found in genesis file: %s\n", oracleAddress)
				return nil
			}
			...
			delegateOracleMsg := oracletypes.NewMsgDelegateFeedConsent(oracleAddress, sdk.ValAddress(key.GetAddress()))

			msgs := []sdk.Msg{msg, delegatePeggyMsg, delegateOracleMsg}
