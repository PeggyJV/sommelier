...
func (msg MsgDelegateFeedConsent) ValidateBasic() error {
	...
	if err != nil || operator.Empty() {
		return sdkerrors.Wrap(sdkerrors.ErrInvalidAddress, "much give valid validator address")
	}
	delegate, err := sdk.AccAddressFromBech32(msg.Delegate)
	if err != nil || delegate.Empty() {
		return sdkerrors.Wrap(sdkerrors.ErrInvalidAddress, "much give valid account address")
	}
	return nil
}

// NewMsgAggregateExchangeRatePrevote returns MsgAggregateExchangeRatePrevote instance
func NewMsgAggregateExchangeRatePrevote(hash []byte, feeder sdk.AccAddress, validator sdk.ValAddress) MsgAggregateExchangeRatePrevote {
...
func (msg MsgAggregateExchangeRateVote) ValidateBasic() error {
	...
	for _, tuple := range exchangeRateTuples {
		...
		if tuple.ExchangeRate.BigInt().BitLen() > 100+sdk.DecimalPrecisionBits {
